...
vectors = Vectors(name='BERT_128D.txt', cache=r'C:\Users\25335\Downloads\summarization_new\.vector_cache')

SOURCE.build_vocab(train_data, vectors=vectors)
TARGET.build_vocab(train_data, vectors=vectors)
print('\n')
...
class Encoder(nn.Module):
    def __init__(self, 
        ...
        #self.tok_embedding = nn.Embedding(input_dim, hid_dim) #original
        self.embedding1 = nn.Embedding(len(SOURCE.vocab), hid_dim,SOURCE.vocab.stoi[SOURCE.pad_token])#glove
    ...
    def forward(self, src, src_mask):
        ...
        src = self.dropout((self.embedding1(src) * self.scale) + self.pos_embedding(pos))#glove
...
class Decoder(nn.Module):
    def __init__(self, 
        ...
        #self.tok_embedding = nn.Embedding(output_dim, hid_dim)#original
        self.embedding2 = nn.Embedding(len(TARGET.vocab), hid_dim,SOURCE.vocab.stoi[SOURCE.pad_token])#glove
    ...
    def forward(self, trg, enc_src, trg_mask, src_mask):
        ...
        #trg = self.dropout((self.tok_embedding(trg) * self.scale) + self.pos_embedding(pos))#original
        trg = self.dropout((self.embedding2(trg) * self.scale) + self.pos_embedding(pos))#glove
...
# need to be changed
# new: plugging in glove for both input and output
pretrained_embeddings1 = SOURCE.vocab.vectors
enc.embedding1.weight.data.copy_(pretrained_embeddings1)
UNK_IDX1 = SOURCE.vocab.stoi[SOURCE.unk_token]
PAD_IDX1 = SOURCE.vocab.stoi[SOURCE.pad_token]
#set the pad and unknown as 0 in the pre-trained emb
enc.embedding1.weight.data[UNK_IDX1] = torch.zeros(128)
enc.embedding1.weight.data[PAD_IDX1] = torch.zeros(128)


pretrained_embeddings2 = TARGET.vocab.vectors
dec.embedding2.weight.data.copy_(pretrained_embeddings2)
UNK_IDX2 = TARGET.vocab.stoi[TARGET.unk_token]
PAD_IDX2 = TARGET.vocab.stoi[TARGET.pad_token]
#set the pad and unknown as 0 in the pre-trained emb
dec.embedding2.weight.data[UNK_IDX2] = torch.zeros(128)#hidden=100
dec.embedding2.weight.data[PAD_IDX2] = torch.zeros(128)
# end
...
#f=open('transformer3_2_result.txt','w', encoding='utf-8')
...
model.load_state_dict(torch.load(r"C:\Users\25335\Downloads\COVID_summarizer\hybrid\model.pt"))
# sample txt
